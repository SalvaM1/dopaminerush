...
# Despertar
const NEGRO_INICIAL: float = 4.0      # segundos de negro con el despertador sonando
const FUNDIDO_DESPERTAR: float = 4.0  # cuanto tarda en aparecer la imagen

# Colapso
const NEGRO_COLAPSO: float = 2.0

# Cuanto atras del punto de camara queda el cuerpo al levantarse
const DISTANCIA_LEVANTARSE: float = 0.5
...
func _levantarse() -> void:
	...
	# El cuerpo da un paso atras AL MISMO TIEMPO, para que no haya corte
	var destino_cuerpo := jugador.global_position
	destino_cuerpo -= jugador.global_transform.basis.z * -DISTANCIA_LEVANTARSE
	tween.tween_property(jugador, "global_position", destino_cuerpo, DURACION_TRANSICION)
